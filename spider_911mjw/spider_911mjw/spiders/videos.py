...
class VideosSpider(scrapy.Spider):
    ...
    def parse(self, response):
        self.logger.info(
            f'base_url: {self.base_url}, page_limit_count: {self.page_limit_count}, video_limit_count: {self.video_limit_count}')
    ...
    def parse_page_count(self, response):
        page_count_str = response.css('.pagination').xpath(
            'ul/li[last()]').css('::text').get()
        ...
        if page_count < 0:
            self.logger.warning(
                f'parse page count failed, set page count is 1')
            page_count = 1
        return page_count
    ...
    def parse_first_videos_page(self, response, top_title, base_page_path):
        ...
        self.logger.info(
            f'found title {top_title}, page_path: {base_page_path}, page_count: {self.page_limit_count}/{page_count}')
    ...
    def parse_videos_page(self, response, top_title):
        ...
        movie_count = 0
        self.logger.info(
            f'parse video url: {response.url}, found movie count: {self.video_limit_count}/{len(movie_list)}')
        for movie_selector in movie_list:
            ...
            if movie_selector.css('.zhuangtai > span::text').get():
                item['status'] = movie_selector.css(
                    '.zhuangtai > span::text').get()
    ...
    def parse_video_info(self, response, top_title, item, id):
        ...
        while i < len(ori_video_info_text_list):
            ...
            if key.endswith(':'):
                ...
                try:
                    ...
                    self.logger.error(
                        f'parse key failed({e}), key({key}), url: {item["url"]}, {i}')
                    self.logger.error(f'video_info1: {video_info_text_list}')
                    self.logger.error(
                        f'video_info2: {ori_video_info_text_list}')
                i += 1
        if len(video_info_text_list) % 2:
            self.logger.error(
                f'{item["name"]}, video_info_text_list: {video_info_text_list}, ori: {ori_video_info_text_list}')
        ...
        while i < len(video_info_text_list):
            ...
            if key in info_name:
                ...
                self.logger.error(
                    f'not found video info key({key}), value({value}), url: {item["url"]}')
                self.logger.error(f'video_info1: {video_info_text_list}')
